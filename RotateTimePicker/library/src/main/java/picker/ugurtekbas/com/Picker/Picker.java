    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (firstRun) {
            ...
            if (hourFormat) {
                ...
                mStr = convertAngleToMinuteString(degrees);
                amPmStr = "";
            } else {
                if (manualAdjust) {
                    //get Minutes
...
//        paint.setTextSize(min / 10);
        ...
        if (showHr)
            canvas.drawText(hStr + amPmStr, dialX, dialY, paint);
        else canvas.drawText(mStr, dialX, dialY, paint);
    ...
    public String convertAngleToMinuteString(double angle) {
        String time = "";
//        double decimalValue = 3.0f - (1.0f/30.0f) * (angle % 360);
//        if (decimalValue < 0)
//            decimalValue += 12.0f;
//
...
//        int minutes = (int)(decimalValue * 60) % 60;
        return String.valueOf((int) Math.ceil(angle / 6));
    }
}